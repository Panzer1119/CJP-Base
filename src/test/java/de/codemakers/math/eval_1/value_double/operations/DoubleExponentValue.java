...
public class DoubleExponentValue extends DoubleOperationValue {
    
    public DoubleExponentValue(Value<Double>... values) {
        super(values);
    }
    ...
    protected Double apply(Value<Double>[] values) {
        ...
        return Stream.of(values).skip(1).map(Value::getValue).reduce(result, (result_, value) -> Math.pow(result_, value));
    }
